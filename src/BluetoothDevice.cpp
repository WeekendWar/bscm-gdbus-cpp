...
BluetoothDevice::BluetoothDevice(GDBusConnection* connection, const std::string& object_path)
    : connection_(connection), object_path_(object_path), connected_(false), services_resolved_(false) {
    if (connection_) {
...
void BluetoothDevice::update_properties() {
    ...
    auto services_resolved_var = get_property(BlueZ::DEVICE_INTERFACE, "ServicesResolved");
...
GVariant* BluetoothDevice::get_property(const std::string& interface, const std::string& property) {
...
bool BluetoothDevice::set_property(const std::string& interface, const std::string& property, GVariant* value) {
    ...
    if (!result) {
        if (error) {
            std::cerr << "Failed to set property " << property << ": " << error->message << std::endl;
            g_error_free(error);
        }
        return false;
    }
...
bool BluetoothDevice::connect() {
    if (!connection_ || connected_) return connected_;
    ...
    if (!result) {
        if (error) {
            std::cerr << "Failed to connect to device: " << error->message << std::endl;
            g_error_free(error);
        }
        return false;
    }
...
bool BluetoothDevice::disconnect() {
    if (!connection_ || !connected_) return true;
...
bool BluetoothDevice::pair() {
    ...
    if (!result) {
        if (error) {
            std::cerr << "Failed to pair with device: " << error->message << std::endl;
            g_error_free(error);
        }
        return false;
    }
...
bool BluetoothDevice::refresh_services() {
    if (!connection_ || !connected_) return false;
    
    // Wait for services to be resolved
    for (int i = 0; i < 100; ++i) {
        update_properties();
        if (services_resolved_) break;
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    
    if (!services_resolved_) {
        Utils::print_with_timestamp("Services not resolved yet, discovering characteristics anyway...");
    }
...
void BluetoothDevice::discover_services_and_characteristics() {
    ...
    while (g_variant_iter_loop(objects_iter, "{&o@a{sa{sv}}}", &object_path, &interfaces_dict)) {
        ...
        if (g_str_has_prefix(object_path, object_path_.c_str())) {
            ...
            while (g_variant_iter_loop(interfaces_iter, "{&s@a{sv}}", &interface_name, &properties_dict)) {
                if (g_strcmp0(interface_name, BlueZ::GATT_CHARACTERISTIC_INTERFACE) == 0) {
                    auto characteristic = std::make_shared<GattCharacteristic>(connection_, object_path);
    ...
    Utils::print_with_timestamp("Discovered " + std::to_string(characteristics_.size()) + " characteristics");
}

std::vector<std::shared_ptr<GattCharacteristic>> BluetoothDevice::get_characteristics() {
...
std::shared_ptr<GattCharacteristic> BluetoothDevice::get_characteristic(const std::string& service_uuid, const std::string& char_uuid) {
    for (const auto& pair : characteristics_) {
        if (g_ascii_strcasecmp(pair.second->get_uuid().c_str(), char_uuid.c_str()) == 0) {
            return pair.second;
        }
    }
    return nullptr;
...
std::shared_ptr<GattCharacteristic> BluetoothDevice::get_characteristic_by_path(const std::string& char_path) {
...
bool BluetoothDevice::read_characteristic(const std::string& service_uuid, const std::string& char_uuid, std::vector<uint8_t>& data) {
...
bool BluetoothDevice::write_characteristic(const std::string& service_uuid, const std::string& char_uuid, const std::vector<uint8_t>& data) {
    auto characteristic = get_characteristic(service_uuid, char_uuid);
    if (!characteristic) {
        Utils::print_with_timestamp("Characteristic not found: " + char_uuid);
        return false;
...
bool BluetoothDevice::subscribe_to_notifications(const std::string& service_uuid, const std::string& char_uuid, NotificationCallback callback) {
    auto characteristic = get_characteristic(service_uuid, char_uuid);
    if (!characteristic) {
        Utils::print_with_timestamp("Characteristic not found: " + char_uuid);
        return false;
...
bool BluetoothDevice::unsubscribe_from_notifications(const std::string& service_uuid, const std::string& char_uuid) {
    auto characteristic = get_characteristic(service_uuid, char_uuid);
    if (!characteristic) {
        Utils::print_with_timestamp("Characteristic not found: " + char_uuid);
        return false;
...
void BluetoothDevice::print_device_info() {
    ...
    std::cout << "Services Resolved: " << (services_resolved_ ? "Yes" : "No") << std::endl;
...
void BluetoothDevice::print_services_and_characteristics() {
    if (characteristics_.empty()) {
        Utils::print_with_timestamp("No characteristics discovered. Make sure device is connected and services are resolved.");
        return;
    }
...
void BluetoothDevice::update_connection_state(bool connected) {
    if (connected_ != connected) {
        ...
        Utils::print_with_timestamp("Device " + address_ + " connection state changed: " + 
