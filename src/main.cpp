...
private:
    ...
    void print_help() {
        ...
        std::cout << "  power on/off                - Power on/off the Bluetooth adapter" << std::endl;
        std::cout << "  scan [service_uuid]         - Start scanning for devices (optionally filter by service UUID)" << std::endl;
        std::cout << "  stop                        - Stop scanning" << std::endl;
        std::cout << "  list                        - List discovered devices" << std::endl;
        std::cout << "  connect <address>           - Connect to device by MAC address" << std::endl;
        std::cout << "  disconnect                  - Disconnect from current device" << std::endl;
        std::cout << "  services                    - List services and characteristics of connected device" << std::endl;
        std::cout << "  read <service_uuid> <char_uuid>  - Read characteristic value" << std::endl;
        std::cout << "  write <service_uuid> <char_uuid> <hex_data>  - Write to characteristic" << std::endl;
        std::cout << "  notify <service_uuid> <char_uuid> [on/off]   - Enable/disable notifications" << std::endl;
        std::cout << "  device                      - Show current device info" << std::endl;
        std::cout << std::endl;
    }
    ...
    void handle_scan_command(const std::vector<std::string>& args) {
        if (args.size() > 1) {
            ...
            if (manager_.start_discovery(service_uuids)) {
                Utils::print_with_timestamp("Started scanning for devices with service UUID: " + args[1]);
            } else {
    ...
    void handle_connect_command(const std::vector<std::string>& args) {
        ...
        Utils::print_with_timestamp("Connecting to " + device->get_name() + " (" + args[1] + ")...");
        if (device->connect()) {
    ...
    void handle_read_command(const std::vector<std::string>& args) {
        ...
        if (args.size() < 3) {
            std::cout << "Usage: read <service_uuid> <characteristic_uuid>" << std::endl;
            return;
        }
        
        std::vector<uint8_t> data;
        if (current_device_->read_characteristic(args[1], args[2], data)) {
            Utils::print_with_timestamp("Read " + std::to_string(data.size()) + " bytes: " + 
    ...
    void handle_write_command(const std::vector<std::string>& args) {
        ...
        if (args.size() < 4) {
            std::cout << "Usage: write <service_uuid> <characteristic_uuid> <hex_data>" << std::endl;
            return;
        }
        
        auto data = Utils::hex_string_to_bytes(args[3]);
        if (current_device_->write_characteristic(args[1], args[2], data)) {
            Utils::print_with_timestamp("Write successful: " + Utils::bytes_to_hex_string(data));
        } else {
    ...
    void handle_notify_command(const std::vector<std::string>& args) {
        ...
        if (args.size() < 3) {
            std::cout << "Usage: notify <service_uuid> <characteristic_uuid> [on/off]" << std::endl;
            return;
        }
        ...
        if (enable) {
            auto callback = [this, args](const std::string& char_path, const std::vector<uint8_t>& data) {
                Utils::print_with_timestamp("NOTIFICATION [" + args[2] + "]: " + 
                                          Utils::bytes_to_hex_string(data));
            };
            
            if (current_device_->subscribe_to_notifications(args[1], args[2], callback)) {
        ...
        while (true) {
            ...
            if (command == "help") {
                ...
                if (args.size() > 1 && args[1] == "on") {
                    ...
                    Utils::print_with_timestamp(std::string("Adapter is ") + 
                ...
                if (current_device_ && current_device_->disconnect()) {
                    ...
                    Utils::print_with_timestamp("Failed to disconnect or no device connected");
                }
